...
def uniformCostSearch(problem):
    ...
    visited = []
    myPQueue.push((startNode, [], 0), 0)
    while not myPQueue.isEmpty():
        currentNode, actions, prevCost = myPQueue.pop()
        if currentNode in visited:
            continue
        visited.append(currentNode)
        if problem.isGoalState(currentNode):
        ...
        for nextNode, action, cost in problem.getSuccessors(currentNode):
            ...
            myPQueue.push((nextNode,newAction, priority), priority)
    util.raiseNotDefined()
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    visited = []
    myPQueue = util.PriorityQueue()
    myPQueue.push((starNode, [], 0), 0)
    while not myPQueue.isEmpty():
        currentNode, actions, preCost = myPQueue.pop()
        if currentNode in visited:
            continue
        visited.append(currentNode)
        if problem.isGoalState(currentNode):
        ...
        for nextNode, action, cost in problem.getSuccessors(currentNode):
            ...
            myPQueue.push((nextNode, newAction, newCost), heuristicCost)
